  ...
  if (response.ok) {
    ...
    if (firstLoading) {
      ...
      sites.forEach((site) => {
        ...
        document
          .querySelector(`[data-id="${site.id}"] .show-more-btn`)
...
function addSiteNameToDom(siteData) {
  if (siteData.top_site === true) {
    // Desktop
    const projectMenuItemTemplate = document.getElementById(
      'project_item-template'
    );

    const cloneProjectMenuItem =
      projectMenuItemTemplate.content.cloneNode(true);

    cloneProjectMenuItem.querySelector('[slot="site-name"]').textContent =
      siteData.name;
    cloneProjectMenuItem.querySelector('[slot="site-id"]').dataset.id =
      siteData.id;

    cloneProjectMenuItem
      .querySelector('[slot="site-name"]')
      .addEventListener('click', () => {
        loadSiteDetails(siteData);
      });

    const desktopReferenceNode = document.querySelector(
      '.projects.desktop .see-all'
    );

    document
      .querySelector('.project-items')
      .append(cloneProjectMenuItem, desktopReferenceNode);

    // Mobile
    const mainMenuItemTemplate = document.getElementById(
      'main-menu_item-template'
    );

    const cloneMainMenuItem = mainMenuItemTemplate.content.cloneNode(true);

    cloneMainMenuItem.querySelector('[slot="site-name"]').textContent =
      siteData.name;
    cloneMainMenuItem.querySelector('[slot="site-id"]').dataset.id =
      siteData.id;
    ...
    const mobileReferenceNode = document.querySelector(
      '.projects.mobile .see-all'
    );

    document
      .querySelector('.main-menu_items')
      .append(cloneMainMenuItem, mobileReferenceNode);
...
function addSiteDetailsToDom(siteData) {
  ...
  cloneSiteDetails.querySelector('[slot="site-name"]').textContent =
    siteData.name;

  // Insert to DOM the site description
  cloneSiteDetails.querySelector('[slot="site-description"]').textContent =
    siteData.description;

  // Insert to DOM the site technos
  siteData.technosFromSite.forEach((techno) => {
    cloneSiteDetails
      .querySelector('[slot="site-technos"]')
      .append(`#${techno.name} `);
  });
...
export function addListSitesToDom(siteData) {
  ...
  // Insert to DOM the site name
  cloneSiteCard.querySelector('[slot="site-card-name"]').textContent =
    siteData.name;

  // Insert to DOM the site technos
  siteData.technosFromSite.forEach((techno) => {
    cloneSiteCard
      .querySelector('[slot="site-card-technos"]')
      .append(`#${techno.name} `);
  });
